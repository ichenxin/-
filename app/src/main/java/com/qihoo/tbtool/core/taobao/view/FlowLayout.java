...
public class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs) {
        ...
        try {
            mFlow = DEFAULT_FLOW;
            mChildSpacing = DEFAULT_CHILD_SPACING;
            mMinChildSpacing = DEFAULT_CHILD_SPACING;

            mChildSpacingForLastRow = SPACING_UNDEFINED;

            mRowSpacing = 0;

            mMaxRows = DEFAULT_MAX_ROWS;
            mRtl = DEFAULT_RTL;
            mGravity = UNSPECIFIED_GRAVITY;
            mRowVerticalGravity = ROW_VERTICAL_GRAVITY_AUTO;
        } finally {
            a.recycle();
        }
    }
